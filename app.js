...
function getClusterName(clusterId, totalClusters){
  const names = ['Ouro', 'Prata', 'Bronze', 'Ferro', 'Barro', 'Lama', 'Esterco', 'Nem Olhe'];
...
function renderClusterPlot(items, decimals, priorities){
  ...
  // Cores para clusters: Ouro, Prata, Bronze, Ferro, Barro, Lama, Esterco, Nem Olhe
  const colors = [
    '#ffd700', '#c0c0c0', '#cd7f32', '#708090', 
    '#8b4513', '#654321', '#2F4F2F', '#1c1c1c'
  ];
  // Lama (índice 5): marrom escuro #654321
  // Esterco (índice 6): verde escuro #2F4F2F
  ...
  function drawMarker(ctx, x, y, clusterId, color){
    ...
    } else if(name === 'Lama' || name === 'Esterco' || name === 'Nem Olhe'){
      // X
  ...
  const html = primKeys.map(pri => {
    ...
    const secHtml = secKeys.map(sec => {
      ...
      const leafHtml = leaves.map(l => {
        const coord = l.coordStr || '';
        const href = `detalhe.html?sol=${encodeURIComponent(l.nome)}&coord=${encodeURIComponent(coord)}`;
        return `<li><span class="leaf"><a href="${href}" target="_blank" rel="noopener">${l.nome}</a> ${l.coordStr?`(${l.coordStr})`:''}</span> <span class="score">(Z=${l.Zranking.toFixed(decimals)})</span></li>`;
      }).join('');
